...
class KhetBoard(object):
    ...
    def _apply_laser(self, color):
        ...
        if path[-1].type is LaserPathType.hit:
            ...
            if (square.piece.type is PieceType.anubis and square.piece.orientation is Orientation.delta(path[-1].direction,-180)) \
                ...
                piece = self.get(path[-1].position.x, path[-1].position.y)
                return path, piece
        return path, None
    ...
    def apply_laser(self, color):
        ...
        if self.winner is not None:
            raise RuntimeError("Game is already complete " + str(self.winner) + " won")
        results = {}
        path, piece = self._apply_laser(color)
        
        if piece is not None:
            results["destroyed"] = piece
            if piece.piece.type is PieceType.pharaoh:
                self.winner = TeamColor.opposite_color(piece.piece.color)
